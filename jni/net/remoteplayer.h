...
public:
	...
	bool Spawn(uint8_t byteTeam, unsigned int iSkin, VECTOR *vecPos, float fRotation, uint32_t dwColor, uint8_t byteFightingStyle);
	void Remove();
	...
	void SetID(uint16_t playerId) { m_PlayerID = playerId; }
	uint16_t GetID() { return m_PlayerID; }
	...
	void EnterVehicle(uint16_t VehicleID, bool bPassenger);
	void ExitVehicle();

...
typedef struct _ONFOOT_SYNC_DATA
{
	...
	uint8_t byteCurrentKeyAndWeapon;	// +36
...
typedef struct _INCAR_SYNC_DATA
{
	...
	uint8_t byteCurrentKeyAndWeapon;	// +54
	uint8_t byteSirenOn;				// +55
	uint8_t byteLandingGearState;		// +56
	uint16_t TrailerID;				// +57
...
typedef struct _PASSENGER_SYNC_DATA
{
	...
	uint8_t byteCurrentKeyAndWeapon;	// +3
...
public:
	...
	void SpectatePlayer(uint16_t playerId);
	void SpectateVehicle(uint16_t VehicleID);
...
public:
	...
	uint16_t			m_CurrentVehicle;
	uint16_t 			m_LastVehicle;

...
void CRemotePlayer::Process()
{
	...
	if(IsActive())
	{
			...
			m_byteUpdateFromNetwork == UPDATE_TYPE_ONFOOT && !m_pPlayerPed->IsInVehicle())
		{
			...
			uint8_t byteCurrentWeapon = m_ofSync.byteCurrentKeyAndWeapon & 0x3F;
...
void CRemotePlayer::SlerpRotation()
{
	...
	if(m_pPlayerPed) {
		...
		float fZ = atan2(-mat.up.X, mat.up.Y) * 57.295776;
		if(fZ > 360.0f) {
			fZ -= 360.0f;
		}
	...
	uint32_t dwColor, uint8_t byteFightingStyle)
{
	...
	CPlayerPed *pPlayer = pGame->NewPlayer(iSkin, *vecPos, fRotation);
...
void CRemotePlayer::Remove()
{
	...
	if(m_pPlayerPed) {
		bUsedPlayerSlots[m_pPlayerPed->m_bytePlayerNumber] = false;
...
void CRemotePlayer::EnterVehicle(uint16_t VehicleID, bool bPassenger)
{
	CVehiclePool *pVehiclePool = pNetGame->GetVehiclePool();
...
void CRemotePlayer::StateChange(uint8_t byteNewState, uint8_t byteOldState)
{
	if(byteNewState == PLAYER_STATE_DRIVER && byteOldState == PLAYER_STATE_ONFOOT)
	{
		...
		CVehiclePool *pVehiclePool = pNetGame->GetVehiclePool();
		uint16_t sVehicleID = 0xFFFF;
		if(pPlayerPed && pPlayerPed->IsInVehicle() && !pPlayerPed->IsAPassenger() && pVehiclePool)
		{
			sVehicleID = pVehiclePool->FindIDFromGtaPtr(pPlayerPed->GetGtaVehicle());
			if(sVehicleID == m_VehicleID)
			{
				MATRIX4X4 mat;

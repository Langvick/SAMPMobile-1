...
void CNetGame::Process()
{
	...
	if(GetGameState() == GAMESTATE_CONNECTED) {
		...
		if(pPlayer && pCamera) {
			if(pPlayer->IsInVehicle()) {
				...
				pPlayer->TeleportTo(VECTOR(1093.4f, -2036.5f, 82.7106f));
			}
	...
	{
		if(pChatWindow) {
			pChatWindow->AddDebugMessage("Connecting to %d:%d...", m_szHostOrIp, m_iPort);
		}
...
void CNetGame::SendClickTextDraw(uint16_t wTextDrawID)
{
	Log("TextDraw Select: %u", wTextDrawID);
	RakNet::BitStream bsSend;
	bsSend.Write(wTextDrawID);
	m_pRakClient->RPC(RPC_ScrTextDrawSelection, &bsSend, HIGH_PRIORITY, RELIABLE_ORDERED, 0, false, UNASSIGNED_NETWORK_ID, NULL);
}

void CNetGame::SendMenuSelect(uint8_t bRowId)
{
	Log("Menu Select: %u", (uint16_t)bRowId);
	RakNet::BitStream bsSend;
	bsSend.Write(bRowId);
	m_pRakClient->RPC(RPC_MenuSelect, &bsSend, HIGH_PRIORITY, RELIABLE_ORDERED, 0, false, UNASSIGNED_NETWORK_ID, NULL);
}
...
void CNetGame::Packet_PlayerSync(Packet* pkt)
{
	...
	// CURRENT WEAPON/ADDITIONAL KEY (COMPRESSED INTO 1 BYTE)
...
void CNetGame::Packet_VehicleSync(Packet* pkt)
{
	...
	// CURRENT KEY/WEAPON
	bsSync.Read(icSync.byteCurrentKeyAndWeapon);

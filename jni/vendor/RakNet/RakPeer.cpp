...
//SAMPSRV
extern uint16_t usNetGameCookie;
...
//	RPCIndex rpcIndex; // Index into the list of RPC calls so we know what number to encode in the packet //SAMPSRV
...
//	unsigned int bitLength;
	...
	//bool hasTimestamp;
	//bool nameIsEncoded, networkIDIsEncoded; //SAMPSRV
	RPCIndex rpcIndex;
	RPCNode *node;
	RPCParameters rpcParms;
	//NetworkID networkID; //SAMPSRV
	//bool blockingCommand; //SAMPSRV
	...
	incomingBitStream.Read((char*)&uniqueIdentifier, 1); //SAMPSRV
	rpcIndex = rpcMap.GetIndexFromFunctionName(uniqueIdentifier); //SAMPSRV
...
//		char data[MAX_OFFLINE_DATA_LENGTH];
	...
	// We didn't check this datagram to see if it came from a connected system or not yet.
	// Therefore, this datagram must be under 17 bits - otherwise it may be normal network traffic as the min size for a raknet send is 17 bits
	else if ((unsigned char)(data)[0] == ID_OPEN_CONNECTION_REQUEST && length == sizeof(unsigned char)*2+1)//SAMPSRV
	{
		for (i=0; i < rakPeer->messageHandlerList.Size(); i++)
			rakPeer->messageHandlerList[i]->OnDirectSocketReceive(data, length*8, playerId);
	...
	else if ((unsigned char)(data)[0] == ID_OPEN_CONNECTION_COOKIE && length == sizeof(unsigned char)*2+1) // SAMPSRV
	{
		for (i=0; i < rakPeer->messageHandlerList.Size(); i++)
			rakPeer->messageHandlerList[i]->OnDirectSocketReceive(data, length*8, playerId);
		...
		*(uint16_t *)&c[1] = (*(uint16_t *)&data[1] ^ usNetGameCookie);
...
//	unsigned numberOfBytesUsed;
	...
	while (rcs)
	{
		...
		if (rcs->nextRequestTime < timeMS)
		{
			...
			unsigned i;
			for (i=0; i < messageHandlerList.Size(); i++)
				messageHandlerList[i]->OnDirectSocketSend((char*)&c, 24, rcs->playerId); //SAMPSRV
			SocketLayer::Instance()->SendTo( connectionSocket, (char*)&c, 3, rcs->playerId.binaryAddress, rcs->playerId.port ); //SAMPSRV
		}
...
//						packet->playerIndex = ( PlayerIndex ) remoteSystemIndex;
					...
					else if (byteSize > (sizeof(unsigned char) + sizeof(unsigned char)) && (unsigned char)(data)[0] == ID_AUTH_KEY) //SAMPSRV
					{
							packet=AllocPacket(byteSize, data);
							packet->bitSize = bitSize;
							packet->playerId = playerId;

					...
					if(pPlayerPed->GetDistanceFromCamera() <= pNetGame->m_fNameTagDrawDistance)
					{
						...
						if(pNetGame->m_bNameTagLOS) {
							...
							if(pAimCamera) {
								...
								bLOS = pGame->IsLineOfSightClear(VecPos, vecCamera, true, false, false, true, false, false, false);
							}
						}

						if(!pNetGame->m_bNameTagLOS || bLOS) {
	...
	float fDist, float fHealth, float fArmour, bool bAfk)
{
	...
	((void (*)(VECTOR *, VECTOR *, float *, float *, bool, bool))(g_libGTASA + 0x0054EEC0 + 1))(&TagPos, &Out, 0, 0, 0, 0);
